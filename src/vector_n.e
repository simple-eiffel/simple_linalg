...
feature {NONE}
	...
	make_from_array (arr: ARRAY [REAL_64])
		...
		require
			array_valid: arr /= Void and arr.count > 0
		local
			l_idx: INTEGER
		do
			create components.make (1, arr.count)
			l_idx := 1
			across arr as ic loop
...
feature -- Access
	...
	at (index: INTEGER): REAL_64
		...
		require
			index_valid: index >= 1 and index <= dimension
		do
...
feature -- Vector Operations
	...
	dot_product (other: VECTOR_N): REAL_64
		...
		do
			...
			across 1 |..| dimension as i loop
				Result := Result + components [i] * other.at (i)
			end
		ensure
			result_valid: not Result.is_nan
		end
	...
	add (other: VECTOR_N): VECTOR_N
		...
		do
			...
			from l_idx := 1 until l_idx > dimension loop
				l_result_array [l_idx] := components [l_idx] + other.at (l_idx)
				l_idx := l_idx + 1
			end
	...
	scale (factor: REAL_64): VECTOR_N
		...
		do
			create l_result_array.make_filled (0.0, 1, dimension)
			from l_idx := 1 until l_idx > dimension loop
				l_result_array [l_idx] := components [l_idx] * factor
				l_idx := l_idx + 1
			end
			create Result.make_from_array (l_result_array)
		ensure
